      ...
      <Container variant='primary'>
        <div className='flex justify-between items-center relative'>
          ...
          <nav className={`max-md:fixed md:rounded-full z-20 right-0 top-0 max-md:h-screen max-md:pt-16 bg-light  flex max-md:flex-col max-md:w-2/3  ${isOpen ? 'max-md:translate-x-0' : 'max-md:translate-x-full'} transition-all duration-300 ease-in-out`}>
            {ITEMS?.map(item => <a href={item.path} key={item.name} className={`capitalize md:rounded-full text-base text-center block md:text-lg md:px-10 py-3 hover:text-white ${item.name == "home" ? "bg-primary text-white" : "bg-light text-dark"} transition-colors duration-300`}>{item.name}</a>)}
          </nav>
